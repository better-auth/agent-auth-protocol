...
- **Registration** — The process by which a client registers an agent identity under a host on a server. A successful registration results in an `agent_id`, a mode, and an initial permission set.
...
| Identity type | **Host** | The client's persistent identity on a server: a keypair plus metadata. The server associates default capabilities, trust, and optionally a `user_id` with it. |
...
Direct execution metadata is server-defined — the core protocol does not constrain its shape. Clients SHOULD pass all fields through unchanged when describing capabilities or returning grant details to the agent. Clients MAY provide helper tools for execution profiles they recognize (e.g. an HTTP request tool).
...
The core protocol only requires `name` and `description`. Everything else is optional execution metadata for direct callers. For HTTP capabilities, the `http` field SHOULD include at minimum `method` and `url`. Clients that support direct execution SHOULD send the signed JWT as `Authorization: Bearer <token>`.
...
Returns a lightweight listing of capabilities the server offers. Each entry includes only `name`, `description`, and (when authenticated with an agent JWT) `grant_status`. For full capability details including `input` schemas and execution metadata, use `GET /capability/describe` (§6.2.1).
...
Returns the full detail for a single capability, including `input` schema and any execution metadata. This is the complement to the lightweight list endpoint (§6.2) — agents call it when they need to know a capability's input shape or direct execution details.
...
**Example response (HTTP execution metadata):**
...
When `status` is `"active"`, `agent_capability_grants` MUST include full capability details (`description`, `input`, and any execution metadata) on each grant.
...
**Authorization:** Host JWT. Requires `?agent_id=...` query param. The server MUST verify the agent is registered under this host.
...
The `agent_capability_grants` array is the single source of truth for capability state. Each grant has a `status` field: `"active"` (granted and usable), `"pending"` (awaiting approval), or `"denied"` (user rejected). Active grants include full capability details (`description`, `input`, and any execution metadata). Pending and denied grants include only `capability` and `status`.
...
Returns a lightweight listing of available capabilities — names, descriptions, and grant status. All parameters are optional — a bare `list_capabilities()` returns everything the server offers. For full capability details including input schemas and execution metadata, use `describe_capability` (§7.2.1).
...
Returns the full detail for a single capability, including its `input` schema and any execution metadata. Agents call this when they need to know a capability's input shape before executing it, or to re-fetch details that are no longer in context.
...
2. If `status: "granted"` → return the newly granted capabilities with full capability details from the grant response
3. If `status: "pending"` → initiate the approval flow returned by the server (§9). Poll for status until approved or denied. On approval, return full capability details.
...
Clients MAY also support direct execution for capabilities that include execution metadata (§4.2). When a capability has an `http` field, the client MAY construct and send the HTTP request directly per §4.2 instead of going through the execute endpoint. Clients SHOULD prefer the execute endpoint unless there is a specific reason to use direct execution (e.g. streaming, low latency).
...
4. User approves or denies on their device (implementation-specific — the server provides its own approval UI or endpoints)
