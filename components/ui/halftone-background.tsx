...
float fbm(vec2 p) {
  ...
  for (int i = 0; i < 6; i++) { value += amp * snoise(p * freq); freq *= 2.0; amp *= 0.5; }
  return value;
}
void main() {
  ...
  float t = u_time * 0.04;
  vec2 mouseUV = u_mouse; mouseUV.x *= aspect; mouseUV *= 2.5;
  vec2 diff = p - mouseUV; float dist = length(diff); float radius = 0.5;
  if (u_mouse.x > 0.0 && dist < radius) { float strength = smoothstep(radius, 0.0, dist) * 0.2; p += normalize(diff) * strength; }
  if (u_click > 0.01) { vec2 clickUV = u_clickPos; clickUV.x *= aspect; clickUV *= 2.5; vec2 cdiff = p - clickUV; float cdist = length(cdiff); float ripple = sin(cdist * 12.0 - u_time * 8.0) * u_click * 0.4; float falloff = exp(-cdist * 2.0); p += normalize(cdiff + 0.001) * ripple * falloff; }
  ...
  brightness = clamp(brightness, 0.0, 1.0) * 0.18;
  float leftFade = smoothstep(0.0, 0.35, uv.x); brightness *= leftFade;
  vec2 vc = uv - 0.5; float vignette = 1.0 - dot(vc, vc) * 0.6; brightness *= vignette;
