...
| 403 | `dynamic_host_registration_disabled` | First-time host registration is not enabled on this server. The host must be pre-enrolled, or the operator must enable `allowDynamicHostRegistration` |
...
| 403 | `capability_not_granted` | Agent never had an active grant for this capability |
| 403 | `grant_revoked` | A previously-active grant for this capability was revoked. Distinct from `capability_not_granted` so clients can surface a "your access was revoked" message rather than "request access" |
...
### Capability revocation (`POST /agent/revoke-capability`)
...
| Status | Error Code | Meaning |
|--------|------------|---------|
| 401 | `unauthorized` | Caller is not the agent itself, the owning host, or the linked user |
| 404 | `agent_not_found` | Agent ID doesn't exist |
