...
This document is an implementation-oriented comprehensive agent authentication and working draft. It is designed to be adoptable by products and platforms today, with built-in support for existing infrastructure including HTTP APIs, OpenAPI operations, and MCP tools. The protocol has an official [implementations](/docs/implementations) covering all parts of this specification, maintained by the team behind [Better Auth](https://better-auth.com).
...
Agent Auth makes the runtime agent a first-class principal with both properties. Instead of borrowing a user's session or a shared application token, each agent is registered with its own identity, granted capabilities, and lifecycle. Every agent is created under a persistent identity called a **host** ([§2.7](#27-host)), which the **client** ([§6](#6-client)) registers with each server it connects to.
...
- **Agent** — A runtime AI actor scoped to a specific conversation, task, or session that calls external services. Each agent is registered under a host ([§2.7](#27-host)), carries its own server-side identity, granted capabilities, and lifecycle, and authenticates with short-lived signed JWTs ([§4.3](#43-agent-jwt)).
...
- **Registration** — The process by which a client registers an agent identity under a host on a server. A successful registration results in an agent identity and capability grants (see [§5.3](#53-agent-registration)).
...
Each agent is created under a host. This lets the server reason separately about the persistent client identity (`host`) and the runtime actor (`agent`). The details of host establishment, linking, and key distribution are defined in [§2.7](#27-host)–[§2.11](#211-host-states) and [§4.](#4-authentication)
...
sequenceDiagram
    ...
    Note over Agent, AuthServer: Registration Flow
    Agent->>Client: (A) connect (capabilities, reason, mode)
    Client->>AuthServer: (B) Host JWT + agent public key
    AuthServer-->>Client: (C) agent_id + grants (active or pending)
    end

    opt If Pending
    Client->>User: (D) user_code + verification_uri
    ...
    rect rgb(59, 130, 246, 0.05)
    Note over Agent, AuthServer: Execution Flow
    Agent->>Client: (F) execute capability
    Client->>AuthServer: (G) Agent JWT + POST /execute
    Note over AuthServer: (H) validate JWT,<br/>check grants,<br/>execute capability
    AuthServer-->>Client: (I) result
...
- **(F)** Agent calls `execute_capability` ([§6.10](#610-execute_capability)) on the client with a capability name and arguments.
- **(G)** Client sends `POST /capability/execute` ([§5.11](#511-execute-capability)) to the authorization server, authenticated with a short-lived signed agent JWT.
...
Agents that need a raw JWT for direct server-to-server use can still call `sign_jwt` ([§6.5](#65-sign_jwt)). Resource servers that validate agent JWTs directly MAY use introspection ([§5.12](#512-introspect)) or local verification as described later in the spec, but that is an alternative path rather than the standard capability-execution flow shown above.

**Notes:**

- **Discovery is a pre-step for generic clients.** Multi-provider clients MAY call `list_providers` ([§6.2.1](#621-list_providers)), `search_providers` ([§6.2.2](#622-search_providers)), and `discover_provider` ([§6.2.3](#623-discover_provider)) before step (A).

## 2. Core Model

### 2.1 Agent

An agent is a runtime AI actor scoped to a specific conversation, task, or session, that calls external services. Each agent is registered under a host ([§2.7](#27-host)–[§2.11](#211-host-states)), carries its own server-side identity, granted capabilities, and lifecycle, and authenticates with short-lived signed JWTs ([§4.3](#43-agent-jwt)).
...
In autonomous mode (`"autonomous"`), the agent operates without a user in the loop. Capabilities are granted by server policy or administrator approval, not by an end user. Use cases include background workers, scheduled automations, service-to-service agents, and agents that bootstrap before any user interaction. If the agent is later claimed by a user, it transitions to `"claimed"` state ([§2.10](#210-autonomous-agent-claiming)).
...
A server MAY support delegated agents, autonomous agents, or both. Servers advertise supported modes via discovery ([§5.1](#51-discovery)) and MUST reject registration requests for any unsupported mode.
...
| `claimed` | Autonomous agent claimed when its host was linked to a user ([§2.10](#210-autonomous-agent-claiming)). Capabilities revoked, agent archived. Activity history attributed to the user. Terminal state. |
...
The full reactivation mechanics — including which capabilities are auto-approved and which require user approval — are defined in [§5.6.](#56-reactivate)

### 2.6 Revocation

An agent can be revoked by:
- **Itself** — the agent requests its own revocation through the client, which uses its host JWT to call `POST /agent/revoke` ([§5.7](#57-revoke)) on the agent's behalf
...
Revocation is permanent — a revoked agent cannot be reactivated. The specific endpoints and mechanisms are defined in [§5.7](#57-revoke) and [§6.7.](#67-disconnect_agent) Host revocation and its effect on agents is covered in [§2.7](#27-host)–[§2.11.](#211-host-states)

**User deletion.** When a user account is deleted, the server MUST revoke all hosts linked to that user and cascade revocation to all agents under those hosts ([§8.5](#85-revocation-cascade)). The server SHOULD also clean up any pending approvals, capability grants, and activity logs associated with the user in accordance with the server's data retention policy ([§9.2](#92-agent-activity-tracking)).
...
1. **Dynamic registration** — the host is first seen as part of agent registration ([§5.3](#53-agent-registration)). If the host is unknown, the server creates it in `pending` state and waits for user approval.
...
2. **Delegated registration approval** — an unknown host registers a delegated agent ([§5.3](#53-agent-registration)). The user approves the registration, and the server links the host to the approving user.
...
This section defines the capability model itself: what a capability is and what fields it carries. The server endpoint used to list capabilities is defined later in [§5.2.](#52-list-capabilities)
...
Constraints are stored per grant on the agent capability grant record ([§3.3](#33-agent-capability-grant)). They use the capability's **top-level** input field names as keys — nested paths (e.g. `"address.country"`) are not supported. A grant without constraints permits any valid input — constraints are purely additive.

**How constraints are established:**

1. **Agent-proposed.** The agent includes constraints in its capability request ([§5.3](#53-agent-registration), [§5.4](#54-request-capability)). The user sees the scoped request on the approval screen — for example, "Agent wants to transfer $500 to acc_456" — and approves or denies that specific scope.
...
This grant allows transfers of up to $1,000 in USD or EUR, but only to account `acc_456`. Any execution that violates these constraints is rejected with `constraint_violated` ([§5.11](#511-execute-capability)).
...
The standard way to execute a capability is through the execute endpoint `POST /capability/execute` ([§5.11](#511-execute-capability)). The agent calls the client's `execute_capability` tool ([§6.10](#610-execute_capability)) with a `capability` and `arguments`. The client signs an agent JWT and forwards the request to the server. The server validates the JWT, checks that the agent has been granted the capability, executes it, and returns the result.
...
| `public_key` | JWK? | Ed25519 public key (see [§4.1](#41-keypairs)). Present when inline key registration is used. |
...
Clients SHOULD auto-detect the `name` from available context (e.g. device hostname, client name) and pass it as `host_name` during agent registration ([§5.3](#53-agent-registration)).
...
| `mode` | string | `"delegated"` or `"autonomous"`. Immutable after creation ([§2.2](#22-agent-modes)). |
...
| `constraints` | object? | Input constraints on this grant ([§2.13](#213-scoped-grants-constraints)). Keys are input field names; values are exact values or operator objects (`max`, `min`, `in`, `not_in`). Present only when the grant is scoped. |
...
| `reason` | string? | Why this capability was granted, requested, or denied. For denied grants, this is a human-readable explanation returned to the agent ([§5.5](#55-status)). |
...
The `algorithms` field in discovery ([§5.1](#51-discovery)) lists accepted key types for new registrations. In this specification version, the only defined value is `Ed25519`, and conformant clients and servers MUST support it. JWT signing still uses JWA `alg = EdDSA` with the `Ed25519` key type.
...
| `agent_public_key` | JWK | Conditional | Inline agent public key. Required for registration ([§5.3](#53-agent-registration)) unless `agent_jwks_url` is provided. Not required on other endpoints. |
| `agent_jwks_url` | string | Conditional | JWKS URL for the agent's public keys. Required for registration ([§5.3](#53-agent-registration)) unless `agent_public_key` is provided. Not required on other endpoints. |
...
- **Inline-key hosts** (`host_public_key`): The server identifies the host by `iss` (the key's thumbprint). For known hosts, `iss` MUST match the stored identifier. Key rotation via [§5.9](#59-rotate-host-key) updates the stored identifier atomically.
- **JWKS-based hosts** (`host_jwks_url`): The server identifies the host by the `host_jwks_url` value, which is stable across key rotations. The `iss` claim MUST still match the thumbprint of the signing key (identified by `kid` in the JWT header), but when the host rotates its signing key, `iss` changes. The server resolves this by looking up the host by `host_jwks_url`, then re-fetching the JWKS to discover the new key (see [§8.7](#87-host-key-rotation)).
...
Host and agent JWKS URLs MUST point to different endpoints. Servers that fetch JWKS URLs MUST apply URL fetch protections ([§8.12](#812-url-fetch-protection)) and SHOULD cache previously fetched keys. Servers MUST accept host JWTs with or without agent key claims on non-registration endpoints.
...
12. If the matching grant carries `constraints` ([§2.13](#213-scoped-grants-constraints)), validate that the request arguments satisfy all constraints — reject with `constraint_violated` if any are violated
...
6. Check `exp`, `iat` — reject if expired or issued in the future (see [§8.2](#82-jwt-lifetime-and-clock-skew) for clock skew guidance)
7. Check `jti` replay — reject if the `jti` has been seen within the JWT's lifetime window
8. Check host status — reject if `revoked` or `rejected`. For `pending` hosts, only registration ([§5.3](#53-agent-registration)) is allowed
...
For dynamic registration with an unknown host, the server MUST NOT trust the host identity until user approval completes (see [§8.10](#810-approval-gravity)).
...
Clients obtain the server configuration from discovery ([§5.1](#51-discovery)). That configuration includes the server's `issuer` URL. All endpoint paths in this section are relative to that `issuer`.

The Server API is organized into the following groups:

- **Discovery** ([§5.1](#51-discovery)) — how clients find the server's configuration
- **Capabilities and registration flow** ([§5.2](#52-list-capabilities)–[§5.4](#54-request-capability)) — capability discovery, registration, and capability requests
- **Lifecycle operations** ([§5.5](#55-status)–[§5.10](#510-revoke-host)) — status, reactivation, revocation, and key rotation
- **Execute capability** ([§5.11](#511-execute-capability)) — standard capability execution endpoint
- **Introspection** ([§5.12](#512-introspect)) — token introspection for resource servers
- **Error format** ([§5.13](#513-error-format)) — standard error responses
- **Resource server challenge** ([§5.14](#514-resource-server-challenge-optional)) — optional WWW-Authenticate hint for agents
...
|---|---|---|---|
| `version` | string | Yes | Version of this protocol draft implemented by the server (e.g. `"1.0-draft"`). See [§5.1.1](#511-versioning) for versioning. |
...
| `approval_methods` | string[] | Yes | How user approval works. Defined core values: `"device_authorization"` (RFC 8628), `"ciba"` (Client Initiated Backchannel Authentication). Servers MAY include additional custom methods defined by extension profiles (see [§10.9.3](#1093-server-defined-approval-profile)). Clients SHOULD ignore methods they don't recognize. |
...
Capabilities are listed via `/capability/list` ([§5.2](#52-list-capabilities)), described in detail via `/capability/describe` ([§5.2.1](#521-describe-capability)), and executed through `POST /capability/execute` ([§5.11](#511-execute-capability)). The discovery document covers identity and authorization configuration only.

Approval methods are negotiated through `POST /agent/register` ([§5.3](#53-agent-registration)), `POST /agent/request-capability` ([§5.4](#54-request-capability)), and `POST /agent/reactivate` ([§5.6](#56-reactivate)). The server communicates the chosen method in the returned approval object — approval initiation is not a separate discovery-advertised endpoint in the core protocol.

**Caching.** Servers SHOULD include HTTP `Cache-Control` headers on the discovery response per [RFC 9111](https://datatracker.ietf.org/doc/html/rfc9111), following the metadata caching model established by [RFC 9728 §7.10](https://datatracker.ietf.org/doc/html/rfc9728#section-7.10) (OAuth 2.0 Protected Resource Metadata). A `max-age` of 3600 (one hour) is RECOMMENDED for most deployments. Clients MUST respect standard HTTP cache directives when present. When no `Cache-Control` header is present, clients SHOULD apply a default cache lifetime of one hour. See [§10.5](#105-discovery-caching) for client-side caching guidance.
...
Returns a lightweight listing of capabilities the server offers. Each entry includes only the capability name, description, and, when authenticated with an agent JWT, grant status. For full capability details, including input and output schemas, use `GET /capability/describe` ([§5.2.1](#521-describe-capability)).
...
| Field | Type | Required | Description |
|---|---|---|---|
| `capabilities` | array | Yes | Array of capability summary objects. Each object carries the fields defined in [§2.12](#212-capabilities) (`name`, `description`, and `grant_status` when authenticated with an agent JWT). |
...
Returns the full detail for a single capability, including its input and output schemas. This is the complement to the lightweight list endpoint ([§5.2](#52-list-capabilities)) — agents call it when they need to know a capability's input shape before executing it, or to understand the shape of the data it returns.

**Authorization:** Same three modes as `GET /capability/list` ([§5.2](#52-list-capabilities)). When authenticated with an agent JWT, the response includes grant status.
...
The response is a single capability object with all fields defined in [§2.12](#212-capabilities) — `name`, `description`, `input` (when the capability takes arguments), `output` (when defined), and `grant_status` (when authenticated with an agent JWT). Clients MUST treat a missing `input` the same as an empty schema.

If the capability name does not exist, the server MUST return `404` with error code `capability_not_found`.

**Capability schema evolution:** Capability input and output schemas may change over time (e.g. new required fields, changed types). The protocol does not define a versioning mechanism for capabilities — capability names are opaque identifiers ([§2.14](#214-capability-naming)) and servers are free to adopt naming conventions (e.g. `transfer_money_v2`) if needed. When a capability's schema changes and an agent sends arguments conforming to a stale schema, the server SHOULD reject the request with `400 invalid_request`. Clients SHOULD re-fetch `GET /capability/describe` ([§5.2](#52-list-capabilities)) on schema-related errors and present the updated schema to the agent.
...
**Authorization:** Host JWT with agent key claims ([§4.2](#42-host-jwt)). The host JWT MUST include `agent_public_key` or `agent_jwks_url` for registration.
...
| `preferred_method` | string | No | Preferred approval method (e.g. `"device_authorization"`, `"ciba"`). This is a hint — the server makes the final decision and returns the chosen method in the approval object ([§7.5](#75-method-selection)). Values correspond to the approval methods list in the discovery response ([§5.1](#51-discovery)), including any extension-defined methods the client understands. |
...
The response returns a subset of agent fields from [§3.2](#32-agent), plus grant and approval data:
...
| `approval` | object | Conditional | Approval flow object ([§7.5](#75-method-selection)). Present when `status` is `"pending"`. |

**Capability grant fields in registration, status, reactivation, and request-capability responses:**

Each grant object carries the grant fields from [§3.3](#33-agent-capability-grant) (`capability`, `status`, `constraints`, `granted_by`, `reason`) plus, for active grants, the capability's own fields from [§2.12](#212-capabilities) (`description`, `input`, `output`). The conditional rules:
...
- **Denied grants**: Include `capability`, `status`, and an optional human-readable `reason` (see [§5.5](#55-status) for the denied grant format).

The introspect endpoint ([§5.12](#512-introspect)) is an exception: it returns compact grant objects (`capability` and `status` only) because resource servers need to verify grants, not capability schemas.
...
The client polls `GET /agent/status` ([§5.5](#55-status)) at the `interval` rate until the agent's status changes from `"pending"` to `"active"` (or the request expires).

**Partial approval.** When a registration requests multiple capabilities, the user MAY approve some and deny others. The server MUST reflect this in the `agent_capability_grants` array — each grant carries its own `status` (`"active"`, `"pending"`, or `"denied"`). The agent's top-level `status` is `"active"` as long as at least one capability was approved. A fully denied registration (all capabilities denied) sets the agent status to `"active"` with an empty or all-denied grants array. This is intentional: the agent identity exists and can request additional capabilities via [§5.4](#54-request-capability) without re-registering. Clients SHOULD check the grants array after registration and inform the agent which capabilities were denied. Servers SHOULD include a `reason` field (string) on denied grants so the agent (or user) understands why (see [§3.3](#33-agent-capability-grant) `reason` field).
...
| Field | Type | Required | Description |
|---|---|---|---|
| `capabilities` | (string \| object)[] | Yes | Additional capabilities to request. Each element is either a capability name string (unconstrained) or an object containing a capability name and optional constraints for scoped grants ([§2.13](#213-scoped-grants-constraints)). |
...
| `agent_capability_grants` | array | Yes | Array of grant objects for the requested capabilities only (grant fields as described in [§5.3](#53-agent-registration)). |
| `approval` | object | Conditional | Approval flow object ([§7.5](#75-method-selection)). Present when any grant is `"pending"`. |
...
The status endpoint is the comprehensive view of an agent. It returns the full agent data model ([§3.2](#32-agent)) plus all capability grants (grant fields as described in [§5.3](#53-agent-registration)):
...
| `mode` | string | Yes | `"delegated"` or `"autonomous"`. |
| `agent_capability_grants` | array | Yes | Array of all capability grant objects for this agent (grant fields as described in [§5.3](#53-agent-registration)). |
...
The capability grants array in the status response reflects the server's current capability state for this agent — grant fields are as described in [§5.3.](#53-agent-registration) The status endpoint is the only endpoint guaranteed to return all grant metadata including `granted_by`.
...
Reactivates an expired agent. Reactivation is a security checkpoint: the agent's capabilities decay to its host's default capability set. Any previously escalated capabilities are lost and must be re-requested through `POST /agent/request-capability` ([§5.4](#54-request-capability)).
...
4. Determine the host's current default capabilities and grant them to the agent, following the same auto-approval logic as registration ([§5.3](#53-agent-registration)): if the host is linked and the capabilities fall within its defaults, auto-approve. Otherwise, return `pending` with an approval flow.
...
The response returns the same fields as the status endpoint ([§5.5](#55-status)) — all agent fields from [§3.2](#32-agent) plus `agent_capability_grants` (grant fields as described in [§5.3](#53-agent-registration)) — plus:
...
The client polls `GET /agent/status` ([§5.5](#55-status)) at the `interval` rate until the agent's status changes from `"pending"` to `"active"` (approved) or the approval expires.
...
This endpoint is for inline key hosts only. JWKS-based hosts rotate by updating the key at their JWKS URL — servers pick up the new key on their next fetch. See [§8.7](#87-host-key-rotation) for rotation security considerations.
...
| `status` | string | Yes | Always `"revoked"`. |
| `agents_revoked` | number | Yes | Count of agents revoked as part of the cascade ([§8.5](#85-revocation-cascade)). |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `data` | any | Conditional | The capability's result. Present for sync responses. When the capability defines an output schema ([§2.12](#212-capabilities)–[§2.15](#215-execution)), this value SHOULD conform to it. |
...
| `error` | object | Conditional | Error details per [§5.13.](#513-error-format) Present when `status` is `"failed"`. |
...
| `error` | Error object per [§5.13](#513-error-format) | An error occurred during streaming execution. |
...
If the agent's grant for the requested capability carries constraints ([§2.13](#213-scoped-grants-constraints)), the server MUST validate the supplied arguments against those constraints before executing. If any argument violates a constraint, the server MUST reject the request with `403 constraint_violated`. The error response SHOULD include a violations array listing which constraints were violated:
...
| 400 | `unknown_constraint_operator` | Grant contains an unrecognized constraint operator ([§2.13](#213-scoped-grants-constraints)) |
| 403 | `capability_not_granted` | Agent does not have an active grant for this capability |
| 403 | `constraint_violated` | Arguments violate the grant's constraints ([§2.13](#213-scoped-grants-constraints)) |
...
The response includes a subset of agent fields from [§3.2](#32-agent), plus compact grants. Unlike other endpoints ([§5.3](#53-agent-registration)), introspect returns grant objects with only `capability` and `status` — no `description`, `input`, `output`, or `constraints` — because resource servers need to verify grants, not capability schemas.
...
The auth server performs the full verification flow ([§4.5](#45-verification)) and returns an active/inactive response.
...
| 400 | `unknown_constraint_operator` | A constraint contains an operator the server does not recognize ([§2.13](#213-scoped-grants-constraints)) | Remove or replace the unknown operator and retry |
...
| 403 | `agent_expired` | Agent session has expired | Reactivate via `POST /agent/reactivate` ([§5.6](#56-reactivate)) |
...
| Any agent-authenticated | 403 | `constraint_violated` | Arguments violate the grant's constraints ([§2.13](#213-scoped-grants-constraints)). Response SHOULD include a `violations` array. |
...
| `GET /capability/list` | 401 | `authentication_required` | Server requires auth to list capabilities ([§5.2](#52-list-capabilities)) |
...
For device authorization endpoints ([§7.1](#71-device-authorization-rfc-8628)), error responses follow [RFC 8628 §3.5](https://datatracker.ietf.org/doc/html/rfc8628#section-3.5). For CIBA endpoints ([§7.2](#72-cibahttpsopenidnetspecsopenid-client-initiated-backchannel-authentication-core-1_0html-client-initiated-backchannel-authentication)), error responses follow [OpenID CIBA Core §13](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html#rfc.section.13).
...
|---|---|---|
| `discovery` | Yes | Absolute URL to the authorization server's `/.well-known/agent-configuration` endpoint ([§5.1](#51-discovery)) |
...
- Clients that receive an `AgentAuth` challenge SHOULD fetch the discovery URL if no active agent exists for that server, then proceed through registration ([§5.3](#53-agent-registration)) before retrying the original request.
- If the agent JWT is present but the agent lacks the required capability, the resource server SHOULD return `403` with the `capability_not_granted` error code ([§5.13](#513-error-format)) instead of a `401` challenge.

This mechanism is OPTIONAL. Agents SHOULD prefer proactive discovery via `/.well-known/agent-configuration` or registry search (`search_providers`, [§6.2.2](#622-search_providers)) when possible. The challenge header exists as a fallback for cases where the agent has no prior knowledge of the service's auth configuration.
...
1. Query a registry ([§6.11](#611-registry)) to find matching services.
...
Returns a lightweight listing of available capabilities — names, descriptions, and grant status. All parameters are optional — a bare `list_capabilities()` returns everything the server offers. For full capability details including input schemas, use `describe_capability` ([§6.3.1](#631-describe_capability)).
...
1. Call `GET /capability/list` ([§5.2](#52-list-capabilities)) with a host JWT, or an agent JWT if `agent_id` is provided.
...
1. Call `GET /capability/describe?name=...` ([§5.2.1](#521-describe-capability)) with a host JWT, or an agent JWT if `agent_id` is provided.
...
Mode choice matters at registration time. A server MAY support delegated agents, autonomous agents, or both. Servers advertise which modes they support via discovery ([§5.1](#51-discovery)) and MUST reject registration requests for any unsupported mode. How mode affects host linking is covered in [§2.7](#27-host)–[§2.11.](#211-host-states)
...
| `capabilities` | (string \| object)[] | No | Capabilities to request. Each element is either a capability name string (unconstrained) or an object with a capability name and optional constraints for scoped grants ([§2.13](#213-scoped-grants-constraints)). If omitted, the agent is created with no capabilities — the agent can request them later via `request_capability` ([§6.6](#66-request_capability)). |
| `mode` | string | No | `"delegated"` (default) or `"autonomous"` |
| `reason` | string | No | Human-readable reason for the request. Displayed to the user on the approval screen. |
| `preferred_method` | string | No | Preferred approval method hint (e.g. `"device_authorization"`, `"ciba"`). Passed through to the server on `POST /agent/register` ([§5.3](#53-agent-registration)). |
| `login_hint` | string | No | Optional user identifier hint for approval methods like CIBA. Passed through to the server on `POST /agent/register` ([§5.3](#53-agent-registration)). |
| `binding_message` | string | No | Optional short approver-facing message for approval methods that support it. Passed through to the server on `POST /agent/register` ([§5.3](#53-agent-registration)). |
...
3. Sign a host JWT containing the agent's public key (see [§4.2](#42-host-jwt) for JWT construction)
4. Call `POST /agent/register` ([§5.3](#53-agent-registration)) with the host JWT, requested capabilities, mode, and any approval hints
...
2. Sign an agent JWT (see [§4.3](#43-agent-jwt) for claims and construction). The JWT SHOULD expire within 60 seconds.
3. Return the signed JWT string

The client MAY refuse to sign (e.g. agent is revoked, rate limit exceeded). The client MAY restrict the JWT to specific capabilities even if the agent didn't request it. See [§4.4](#44-proof-of-possession-optional) for optional proof-of-possession profiles.
...
|---|---|---|---|
| `agent_id` | string | Yes | The agent's ID |
| `capabilities` | (string \| object)[] | Yes | Additional capabilities to request. Each element is either a capability name string (unconstrained) or an object with a capability name and optional constraints for scoped grants ([§2.13](#213-scoped-grants-constraints)). |
| `reason` | string | No | Why the agent needs these capabilities |
| `preferred_method` | string | No | Preferred approval method hint (e.g. `"device_authorization"`, `"ciba"`). Passed through to the server on `POST /agent/request-capability` ([§5.4](#54-request-capability)). |
| `login_hint` | string | No | Optional user identifier hint for approval methods like CIBA. Passed through to the server on `POST /agent/request-capability` ([§5.4](#54-request-capability)). |
| `binding_message` | string | No | Optional short approver-facing message for approval methods that support it. Passed through to the server on `POST /agent/request-capability` ([§5.4](#54-request-capability)). |

**Behavior:**

1. Sign an agent JWT and call `POST /agent/request-capability` ([§5.4](#54-request-capability))
2. If `status: "active"` → return the newly granted capabilities with full capability details from the grant response
3. If `status: "pending"` → initiate the approval flow returned by the server ([§7](#7-approval-methods)). Poll for status until the requested grants are approved or denied. On completion, return the current agent status and grant details.
...
As with the server status response ([§5.5](#55-status)), active grants include full capability details. Pending and denied grants include only `capability` and `status`.
...
2. Sign a host JWT and call `POST /agent/revoke` ([§5.7](#57-revoke))
3. Remove the stored connection data (keypair, config)
4. Return confirmation

### 6.8 reactivate_agent

Reactivates an expired agent connection. The agent keeps its identity (`agent_id`) and keypair, but its capabilities decay to the host's defaults — any previously escalated capabilities are lost and must be re-requested via `request_capability` ([§6.6](#66-request_capability)).
...
1. Look up the host identity associated with this agent connection
2. Sign a host JWT and call `POST /agent/reactivate` ([§5.6](#56-reactivate)) with the `agent_id`
...
4. If `status: "pending"` → initiate the approval flow returned by the server ([§7](#7-approval-methods)). Poll for status until the agent is approved or the approval expires. On approval, update stored grants and return agent_id + granted capabilities (including full details). On rejection or timeout, report the outcome to the agent.
...
Executes a granted capability through the server's execute endpoint ([§5.11](#511-execute-capability)). The client signs an agent JWT and sends the capability name and arguments to `POST /capability/execute`. The server validates the JWT, checks grants, executes the capability, and returns the result.
...
1. Sign an agent JWT scoped to this capability with `aud` set to the target server's `issuer` URL. See [§8.18](#818-cross-server-capability-confusion) for cross-server `aud` enforcement requirements.
2. Call `POST /capability/execute` ([§5.11](#511-execute-capability)) with the agent JWT, `capability`, and `arguments`
...
Each entry in `providers` is a standard discovery document ([§5.1](#51-discovery)). The client MAY normalize these entries into its `search_providers` response shape, but it SHOULD retain the underlying provider configurations the same way it retains directly discovered ones.
...
Servers MUST support device authorization ([§7.1](#71-device-authorization-rfc-8628)) as the baseline approval method. Servers SHOULD also support CIBA ([§7.2](#72-cibahttpsopenidnetspecsopenid-client-initiated-backchannel-authentication-core-1_0html-client-initiated-backchannel-authentication)) — it provides a better user experience when the server already knows the user's identity. Other methods are OPTIONAL.

**Approval object schema:**

The approval object appears in responses from `POST /agent/register` ([§5.3](#53-agent-registration)), `POST /agent/request-capability` ([§5.4](#54-request-capability)), and `POST /agent/reactivate` ([§5.6](#56-reactivate)) when the request requires user consent. The object always includes a `method` field indicating which approval method the server selected, plus method-specific fields:
...
| `notification_url` | string | All | No | SSE endpoint for real-time approval updates ([§10.9.2](#1092-real-time-approval-notification-profile)). |
...
4. Client polls `GET /agent/status` ([§5.5](#55-status)) at the `interval` rate until the agent's status changes from `"pending"` to `"active"` (approved), `"rejected"` (denied), or the request expires

**Agent Auth-specific behavior:**

- **Pending registration retries are idempotent.** See [§5.3](#53-agent-registration) "Idempotency and retry semantics" for the authoritative definition.
...
In this protocol, CIBA is not a separate initiation flow. The client still starts with `POST /agent/register` ([§5.3](#53-agent-registration)), `POST /agent/request-capability` ([§5.4](#54-request-capability)), or `POST /agent/reactivate` ([§5.6](#56-reactivate)). If the server chooses CIBA, it returns `approval.method = "ciba"` in the normal approval object.
...
The server notifies the user through its notification channel (push notification, email, SMS, in-app, etc.), and the client polls `GET /agent/status` ([§5.5](#55-status)) at the returned `interval` rate until the agent or requested grants leave `pending` state.
...
Polling is the baseline for all core approval methods. Real-time approval notification is defined as an extension profile in [§10.9.2.](#1092-real-time-approval-notification-profile) Core clients MUST function correctly using polling alone.

### 7.4 Server-Defined

Custom approval methods are not part of the core protocol. They are defined, if at all, by extension profiles such as the Server-Defined Approval Profile ([§10.9.3](#1093-server-defined-approval-profile)).

### 7.5 Method Selection

When a server supports multiple approval methods, it decides which one to use based on context. Clients MAY include a preferred-method hint in registration ([§5.3](#53-agent-registration)) and capability requests ([§5.4](#54-request-capability)) to indicate which approval method they prefer. The server SHOULD consider this hint but makes the final decision and returns the chosen method in the approval object. The server MAY ignore the preferred-method hint entirely — for example, if the preferred method is unavailable, not supported, or inappropriate for the request context.
...
Inline key hosts rotate via `POST /host/rotate-key` ([§5.9](#59-rotate-host-key)). Since the client does not maintain a durable registry of every server a host is registered on, inline rotation is limited to servers the client currently knows about. For key compromise, revoke the host entirely — a compromised key means you cannot trust the rotation request itself.
...
Because `iss` is the JWK thumbprint of the signing key, JWKS key rotation changes the host's `iss` value. The server updates its stored host identifier when it processes a host JWT signed with the new key ([§4.5.1](#451-host-jwt-verification) step 3). However, there is a race window: agent JWTs may arrive with the new `iss` before any host JWT has triggered the update. Servers MUST handle this by falling back to the agent's `sub` to resolve the parent host when `iss` is unrecognized ([§4.5](#45-verification) step 4). Servers SHOULD also retain the previous host identifier for a grace period matching the JWKS cache TTL.
...
- **Rotate the host key** via [§5.9](#59-rotate-host-key) (inline-key hosts), locking out the legitimate owner
- **Impersonate all future host-authenticated operations** — status checks, capability requests, agent management

The inline key rotation endpoint ([§5.9](#59-rotate-host-key)) is unusable during a host key compromise because it requires authentication with the current (compromised) key. The attacker can use it; the legitimate owner cannot.

**Response:**

1. **Revoke the host immediately.** The user (or server admin) MUST revoke the host through an out-of-band mechanism (dashboard, admin API, support). Revocation cascades to all agents under the host ([§8.5](#85-revocation-cascade)).
...
Mitigations:
- **Registry-first discovery.** Clients SHOULD resolve new services through a trusted registry ([§6.11](#611-registry)) using `search_providers` rather than arbitrary URLs.
...
- **Registry integrity.** When using registry-based discovery, clients SHOULD verify that the `issuer` in the fetched discovery document matches the `issuer` from the registry result. A mismatch indicates a poisoned registry entry or a man-in-the-middle attack. See [§6.11](#611-registry) for registry-side protections.
- **Algorithm downgrade protection.** See [§8.16.](#816-discovery-downgrade-attacks)
...
For discovery-specific protections (allowlists, user confirmation, TLS verification), see [§8.9.](#89-discovery-security)
...
- The response MUST be valid JSON conforming to [RFC 7517 [§5](#5-server)](https://datatracker.ietf.org/doc/html/rfc7517#section-5). Reject responses that do not parse as a JWK Set.
...
**Server-returned URL validation.** The protections above cover URLs that clients and servers *fetch from external sources*. Clients MUST also validate URLs that the *server returns* to them — `status_url` ([§5.11](#511-execute-capability) async polling), `verification_uri` ([§7.1](#71-device-authorization-rfc-8628) device authorization), and `notification_url` (§10.8.2 real-time approval). A malicious or compromised server could return URLs that:
...
- **IP filtering.** Clients SHOULD block `status_url` values that resolve to private, loopback, or link-local IP ranges (same filtering as [§8.12](#812-url-fetch-protection) server-side fetches, applied client-side).
...
**Response:** When rate limited, the server MUST return `429` with the `rate_limited` error code ([§5.13](#513-error-format)) and SHOULD include a `Retry-After` header (seconds until the client can retry):
...
The `algorithms` field in discovery ([§5.1](#51-discovery)) tells clients which key types the server accepts. If an attacker can modify the discovery document in transit (e.g. via a compromised cache or DNS hijack), they could remove strong algorithms and force clients to register with weaker ones.

Mitigations:

- **TLS is the primary defense.** Discovery documents are fetched over HTTPS ([§8.9](#89-discovery-security)), which protects against in-transit modification. This threat is only relevant if TLS is compromised or if a client accepts discovery documents over an insecure channel.
...
- **Servers MUST NOT remove algorithms that are still in active use.** Removing an algorithm from `algorithms` MUST NOT invalidate existing registrations ([§4.1](#41-keypairs)), but the server SHOULD NOT remove an algorithm unless it has a documented migration path for existing hosts and agents using it.

### 8.17 Constraint Enforcement Atomicity

Constraints ([§2.13](#213-scoped-grants-constraints)) are validated at the time the server processes the execution request ([§5.11](#511-execute-capability)). However, the underlying data or state may change between validation and actual execution — a time-of-check/time-of-use (TOCTOU) gap. For example, a constraint of `max_amount: 1000` is checked against the request's `amount` field, but if the execution involves multiple steps, the effective amount could differ by the time the operation completes.
...
Each agent JWT includes an `aud` claim set to the server's `issuer` URL ([§4.3](#43-agent-jwt)). This prevents a JWT intended for server A from being accepted by server B — the `aud` check in verification ([§4.5](#45-verification) step 3) rejects it. However, this only works if both sides enforce `aud`:

- **Servers MUST reject JWTs where `aud` does not exactly match their own `issuer` URL.** This is already required ([§4.5](#45-verification) step 3) but is restated here for emphasis.
...
The protocol provides per-agent, per-call traceability by design ([§11](#11-observability)). This is a feature for security and auditing, but also a privacy concern: the server has a complete record of every capability an agent executed, when, and with what inputs.
...
The `jti` replay cache does not need persistence. Replay detection only matters within the effective validity window — the JWT's lifetime plus clock skew tolerance (see [§8.2](#82-jwt-lifetime-and-clock-skew)). With the default 60-second lifetime and 30-second skew, this is 90 seconds. An in-memory cache or probabilistic structure (bloom filter) with a TTL matching the effective window is sufficient. The cache is partitioned by identity (agent ID or host ID) — see [§4.6.](#46-replay-detection)

### 10.4 JWKS Caching

Servers that fetch JWKS URLs SHOULD cache the response and refresh periodically (recommended: every 5 minutes or on `kid` miss). Servers SHOULD handle JWKS URL unavailability gracefully — use previously cached keys and return a clear error if the URL is unreachable and no cached keys exist. For JWKS content validation requirements (key type restrictions, key count limits, `kid` matching), see [§8.12.](#812-url-fetch-protection)

### 10.5 Discovery Caching

The discovery document ([§5.1](#51-discovery)) changes infrequently — endpoint paths, supported algorithms, and registration modes are typically stable across deployments. Clients SHOULD cache the discovery document locally and re-fetch no more than once per hour under normal operation.
...
Clients MUST respect HTTP `Cache-Control` headers when present ([§5.1](#51-discovery)). When no cache headers are provided, clients SHOULD apply a default cache lifetime of one hour. Clients MUST NOT cache indefinitely — a maximum cache lifetime of 24 hours is RECOMMENDED even for stable deployments, to ensure eventual consistency with server-side changes.

Purpose-built clients that skip discovery and use pre-configured endpoints (as noted in [§5.1](#51-discovery)) are not affected by this guidance.
