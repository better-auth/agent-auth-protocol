...
The server's execute endpoint. The client sends execute requests here when an agent calls `execute_capability` (§6.10).

The server validates the agent JWT, checks that the agent has an active grant for the requested capability, executes the capability, and returns the result. The server acts as a gateway so backend service endpoints do not need to validate agent JWTs themselves.
...
|---|---|---|---|
| `agent_id` | string | Yes | The agent's ID |
| `aud` | string | No | The audience URL for the JWT's `aud` claim. If omitted, the client sets `aud` to the server's `issuer` URL. |
