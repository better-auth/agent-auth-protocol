...
- **(G)** Client resolves the capability's location (§2.15) — the capability's `location` if set, or the server's `default_location` from discovery. Signs a short-lived agent JWT with `aud` set to the location URL and sends the request.
