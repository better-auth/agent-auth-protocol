...
- **Host** — The client's persistent identity: a keypair plus metadata registered with each server. A host is not a running process. It may be linked to a user or unlinked.
- **Capability** — A server-offered action or function. Each capability has a `name` and a human-readable description. Capabilities are executed through the server's execute endpoint (`POST /capability/execute`).
...
- **Registration** — The process by which a client registers an agent identity under a host on a server. A successful registration results in an `agent_id`, a mode, and an initial capability set.
...
| Participant | **Client** | The process that exposes protocol tools to AI systems (MCP server, CLI, SDK). Holds a host identity, manages keys, and signs requests. |
| Participant | **Server** | The service's authorization server. Manages registrations, capabilities, approvals, and verification. |
| Identity type | **Host** | The client's persistent identity on a server: a keypair plus metadata. The server associates default capabilities, trust, and optionally a linked user with it. |
...
- **Capabilities are executed through a standard endpoint.** The agent calls `execute_capability` (§7.9), which sends the capability name and arguments to `POST /capability/execute` (§6.11). The server validates the agent JWT, executes the capability, and returns the result.
...
An agent is a runtime AI actor that needs to call external services. Each agent has its own server-side `agent_id`, its own keypair, its own granted capabilities, and its own lifecycle. Agents are registered under hosts (§3) and authenticate with short-lived signed JWTs (§5.3).
...
| `claimed` | Autonomous agent claimed when its host was linked to a user (§3.4). Capabilities revoked, agent archived. Activity history attributed to the user. Terminal state. |
...
A host is the client's persistent identity on a server: a registered keypair plus metadata. Every agent is registered under a host. A host MAY exist without any linked user (unlinked host) or be linked to at most one user (linked host). Hosts carry default capabilities — agents registered under them can be granted these capabilities without additional approval.
...
In both cases, the result is the same host record: a host keypair (or JWKS URL), an optional linked user, and a set of default capabilities for delegated agents. A host without a linked user MAY provision autonomous agents.
...
Operational guidance for the claiming flow — including user-facing presentation, linking UX, and post-claim management — is in Appendix B.1.

### 3.5 Host States

Every host is in exactly one state at any time. Only `active` hosts can be used to register agents. When a host is revoked, all agents under it are also revoked.
...
Capability names are opaque strings at the protocol layer. Clients and agents MUST treat capability names as uninterpreted identifiers for authorization and execution. They MUST NOT derive protocol semantics from delimiters, prefixes, or other naming patterns.

The protocol still benefits from a consistent naming convention. Servers SHOULD publish stable, human-readable capability names that follow a predictable structure when possible.

**Recommended format:** `[namespace:]verb:resource[:qualifier...]`

- `namespace` is OPTIONAL but RECOMMENDED for providers that expect their capabilities to appear in multi-provider clients. It identifies the provider or product, such as `github`, `slack`, or a reverse-domain name like `com.example.crm`.
- `verb` SHOULD describe the primary action, such as `read`, `list`, `create`, `update`, `delete`, or `send`.
- `resource` SHOULD name the primary object the capability operates on, such as `issue`, `message`, `balance`, or `deployment`.
- `qualifier` segments are OPTIONAL and can further specialize the capability when needed.

**Examples:** `github:create:issue`, `slack:send:message`, `bank:read:balance`, `com.example.crm:update:contact`
...
For generic clients that support multiple providers, the client MAY prefix capability names with the provider name when presenting them to the agent (e.g. `create:issue` → `github.create:issue`) to avoid naming collisions. This is a **client-local convention only**. The client MUST map prefixed names back to the original capability objects before execution. The server never sees prefixed client-local identifiers.
...
|---|---|---|
| `capabilities` | No | String array of capability names this JWT is authorized for. If present, the server MUST reject requests for capabilities not in this list — even if the agent has them granted. If absent, the JWT is valid for all the agent's granted capabilities. Clients SHOULD omit this claim when the JWT does not need to restrict capabilities, since the server already tracks granted capabilities per agent. Include it only when the client intentionally limits a specific JWT to a subset of the agent's grants. |
...
- **Resource server challenge** (§6.14) — optional WWW-Authenticate hint for agents
...
| `approval_methods` | string[] | Yes | How user approval works. Defined core values: `"device_authorization"` (RFC 8628), `"ciba"` (Client Initiated Backchannel Authentication). Servers MAY include additional custom methods defined by extension profiles (see §13.8.3). Clients SHOULD ignore methods they don't recognize. |
...
Capabilities are listed via `/capability/list` (§6.2), described in detail via `/capability/describe` (§6.2.1), and executed through `POST /capability/execute` (§6.11). The discovery document covers identity and authorization configuration only.

Approval methods are negotiated through `POST /agent/register` (§6.3), `POST /agent/request-capability` (§6.4), and `POST /agent/reactivate` (§6.6). The server communicates the chosen method in the returned `approval` object — approval initiation is not a separate discovery-advertised endpoint in the core protocol.
...
Returns a lightweight listing of capabilities the server offers. Each entry includes only `name`, `description`, and (when authenticated with an agent JWT) `grant_status`. For full capability details including `input` schemas, use `GET /capability/describe` (§6.2.1).
...
### 6.2.1 Describe Capability

```
GET /capability/describe?name={capability_name}
```

Returns the full detail for a single capability, including its `input` schema. This is the complement to the lightweight list endpoint (§6.2) — agents call it when they need to know a capability's input shape before executing it.

**Authorization:** Same three modes as `GET /capability/list` (§6.2). When authenticated with an agent JWT, the response includes `grant_status`.
...
**Example response (standard capability with input schema):**
...
| `capabilities` | string[] | No | Requested capability names. If omitted, the agent is created with no capabilities — the agent can request them later via `POST /agent/request-capability` (§6.4). |
...
| `preferred_method` | string | No | Preferred approval method (e.g. `"device_authorization"`, `"ciba"`). This is a hint — the server makes the final decision and returns the chosen method in the `approval` object (§9.5). Values correspond to the `approval_methods` array in the discovery response (§6.1), including any extension-defined methods the client understands. |
...
- If a matching agent exists in `pending` state, the server SHOULD treat the request as an idempotent retry and return the existing `agent_id` and current or refreshed approval flow instead of creating a second agent.
...
When `status` is `"active"`, `agent_capability_grants` MUST include full capability details (`description` and `input`) on each grant.
...
The client polls `GET /agent/status` (§6.5) at the `interval` rate to track pending capability requests. When a capability grant's status changes from `"pending"` to `"active"`, it was approved and the status response includes full capability details. When it changes to `"denied"`, it was denied.
...
The `agent_capability_grants` array in the status response reflects the server's current capability state for this agent. Each grant has a `status` field: `"active"` (granted and usable), `"pending"` (awaiting approval), or `"denied"` (user rejected). Active grants include full capability details (`description` and `input`). Pending and denied grants include only `capability` and `status`.
...
The `error` field is a machine-readable code (snake_case string). The `message` field is a human-readable explanation for debugging — clients SHOULD NOT parse it programmatically. Error responses MAY include additional structured fields beyond `error` and `message` to help clients recover. For example, `invalid_capabilities` errors SHOULD include an `invalid_capabilities` field listing the unrecognized capability names:
...
| `GET /capability/list` | 401 | `authentication_required` | Server requires auth to list capabilities (§6.2) |
...
For device authorization endpoints (§9.1), error responses follow [RFC 8628 §3.5](https://datatracker.ietf.org/doc/html/rfc8628#section-3.5). For CIBA endpoints (§9.2), error responses follow [OpenID CIBA Core §13](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html#rfc.section.13).

### 6.14 Resource Server Challenge (Optional)
...
Returns a lightweight listing of available capabilities — names, descriptions, and grant status. All parameters are optional — a bare `list_capabilities()` returns everything the server offers. For full capability details including input schemas, use `describe_capability` (§7.2.1).
...
1. Call `GET /capability/describe?name=...` (§6.2.1) with a host JWT, or an agent JWT if `agent_id` is provided.
...
| `capabilities` | string[] | No | Capability names to request. If omitted, the agent is created with no capabilities — the agent can request them later via `request_capability` (§7.5). |
| `mode` | string | No | `"delegated"` (default) or `"autonomous"` |
| `reason` | string | No | Human-readable reason for the request. Displayed to the user on the approval screen. |
...
| `login_hint` | string | No | Optional user identifier hint for approval methods like CIBA. Passed through to the server on `POST /agent/register` (§6.3). |
| `binding_message` | string | No | Optional short approver-facing message for approval methods that support it. Passed through to the server on `POST /agent/register` (§6.3). |
...
4. Call `POST /agent/register` (§6.3) with the host JWT, requested capabilities, mode, and any approval hints
...
| `login_hint` | string | No | Optional user identifier hint for approval methods like CIBA. Passed through to the server on `POST /agent/request-capability` (§6.4). |
| `binding_message` | string | No | Optional short approver-facing message for approval methods that support it. Passed through to the server on `POST /agent/request-capability` (§6.4). |
...
3. If `status: "pending"` → initiate the approval flow returned by the server (§9). Poll for status until the requested grants are approved or denied. On completion, return the current agent status and grant details.
...
As with the server status response (§6.5), active grants include full capability details. Pending and denied grants include only `capability` and `status`.
...
4. If `status: "pending"` → initiate the approval flow returned by the server (§9). Poll for status until the agent is approved or the approval expires. On approval, update stored grants and return agent_id + granted capabilities (including full details). On rejection or timeout, report the outcome to the agent.
...
Clients SHOULD auto-detect the `name` from available context (e.g. device hostname, client name) and pass it as `host_name` during agent registration (§6.3).
...
| `reason` | string? | Why this capability was granted or requested |
...
Servers MUST support device authorization (§9.1) as the baseline approval method. Servers SHOULD also support CIBA (§9.2) — it provides a better user experience when the server already knows the user's identity. Other methods are OPTIONAL.
...
- **Pending registration retries are idempotent.** A client MAY retry `POST /agent/register` with the same host identity and agent keypair. The server SHOULD recognize the existing pending agent and return the same `agent_id`, along with either the current approval state or a refreshed approval flow.
...
In this protocol, CIBA is not a separate initiation flow. The client still starts with `POST /agent/register` (§6.3), `POST /agent/request-capability` (§6.4), or `POST /agent/reactivate` (§6.6). If the server chooses CIBA, it returns `approval.method = "ciba"` in the normal approval object.
...
The server notifies the user through its notification channel (push notification, email, SMS, in-app, etc.), and the client polls `GET /agent/status` (§6.5) at the returned `interval` rate until the agent or requested grants leave `pending` state.
...
Polling is the baseline for all core approval methods. Real-time approval notification is defined as an extension profile in §13.8.2. Core clients MUST function correctly using polling alone.

### 9.4 Server-Defined

Custom approval methods are not part of the core protocol. They are defined, if at all, by extension profiles such as the Server-Defined Approval Profile (§13.8.3).
...
General guidance:
- **[CIBA](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html)** when the server already knows the user (e.g. host has a `user_id` or the client provides a `login_hint` the server can resolve)
...
Services with OpenAPI documents can derive capability definitions from existing API operations. An adapter maps OpenAPI operations to Agent Auth capabilities. The execute endpoint can proxy to these operations, so existing APIs don't need to validate agent JWTs themselves.
...
#### 13.8.1 Cross-User Grant Profile
...
#### 13.8.2 Real-Time Approval Notification Profile

This profile augments the core polling model with a `notification_url` in the `approval` object:
...
#### 13.8.3 Server-Defined Approval Profile

This profile allows servers to expose approval methods beyond the core `"device_authorization"` and `"ciba"` set. Extension-defined methods appear in discovery's `approval_methods` array and in the returned `approval.method` field.
...
- Servers SHOULD provide the user with visibility into the autonomous agent's activity before completing the claiming process. See Appendix B.1 for operational guidance on the claiming flow.
...
- **MCP server implementation** — the protocol does not define MCP servers. Services that expose MCP capabilities implement their MCP server independently.
...
An active agent discovers it needs more capabilities while working.
...
This appendix collects non-normative guidance for implementers building user-facing surfaces, dashboards, and management workflows around the protocol. Nothing in this appendix changes the wire-level requirements — it describes how to present protocol state to users and operators.

### B.1 Autonomous Agent Claiming

Claiming (§3.4) is a direct consequence of host linking. The protocol requires that autonomous agents transition to `"claimed"` and that activity is attributed to the user, but the user experience around this transition is an implementation concern. The following guidance helps implementers build a responsible claiming flow.

**Pre-claim disclosure.** Before completing the link, servers should present the user with:

- The number of autonomous agents under the host
- A summary of each agent's activity (capabilities used, resources created, time range)
- A clear statement that claiming is irreversible — the autonomous agents become terminal

This gives the user an informed choice. An agent that checked a public weather API is different from one that created database records or sent messages.

**Labeling.** Activity that occurred before the user was linked should be visually distinguished from post-claim activity. For example, a dashboard might label pre-claim actions as "Performed autonomously before you connected" or group them under a separate timeline section. The goal is to prevent users from mistaking autonomous actions for their own.

**Post-claim management.** Once claimed, the user inherits the agent's data. Servers should provide a way for users to:

- View the claimed agent's full activity history
- Delete the claimed agent and its associated data if they don't want it
- Understand that the agent itself is terminal — no further actions will come from it

**Resource transfer.** If the autonomous agent created resources (files, records, subscriptions), the server should transfer ownership to the user. Resources that cannot be transferred (e.g. external side effects) should be documented in the activity summary so the user knows what happened.

### B.2 Approval Screens

When a user approves an agent registration or capability escalation, the approval screen is the primary trust boundary. The protocol provides the data (agent name, requested capabilities, reason, mode) — this section describes how to present it effectively.

**First-time host approval** (unknown host, no linked user):

- Show the host name (from `host_name` in the registration request) and that this is a new, previously unseen connection
- List each requested capability with its human-readable description
- Show the agent's `reason` field if provided — this is freetext from the agent, so display it as a quote, not as trusted content
- Warn if the request includes high-risk capabilities (the server defines what "high-risk" means for its domain)
- Require fresh authentication — a stale session cookie should not be sufficient (§10.11)

**Capability escalation** (active agent requesting additional capabilities):

- Show which capabilities the agent already has alongside the new request, so the user understands the delta
- Show the `reason` field for the escalation
- Highlight if the new capabilities represent a significant increase in privilege (e.g. going from read-only to write access)

**Mode display.** The approval screen should clearly indicate whether the agent is `delegated` (acting on your behalf) or `autonomous` (operating independently). Users should understand the difference — delegated agents stop when the user revokes them; autonomous agents were never tied to a user in the first place.

### B.3 Agent Management Dashboards

Users need a way to see and manage their agents. The protocol provides the data model (§8) — this section describes what a management surface should expose.

**Agent list.** Show all agents associated with the user, grouped by host. For each agent, display:
...
**Per-agent detail.** Allow the user to drill into a specific agent and see:

- Full capability grant history (what was granted, when, by whom)
- Recent activity log (capability executions with timestamps)
- The ability to revoke the agent

**Host management.** Show hosts as "Connected Apps" or similar user-friendly label (§13.6). Allow users to:

- View all agents under a host
- Revoke a host (which cascades to all its agents, §10.5)
- See the host's default capabilities

### B.4 Lifecycle Notifications

The protocol does not define a webhook or event model (§16), but operators will want to be notified of lifecycle events. Common events worth surfacing:

| Event | When to notify |
|---|---|
| New host registered | A previously unknown host attempted to register an agent |
| Agent approved | A pending agent was approved by the user |
| Agent rejected | A pending agent was denied by the user |
| Agent expired | An agent's session TTL or max lifetime elapsed |
| Agent revoked | An agent was permanently revoked |
| Capability escalation requested | An active agent requested additional capabilities |
| Host linked | A host was linked to a user (triggers claiming) |
| Autonomous agents claimed | Autonomous agents were claimed as part of host linking |

How these notifications are delivered (email, webhook, in-app, push) is implementation-specific. The protocol provides the state transitions — the server decides which are worth notifying on and through which channel.

### B.5 Pending Agent Cleanup

Agents and hosts that remain in `pending` state after their approval window expires represent abandoned registration attempts. Servers should clean these up periodically (§13.7).

Recommended defaults:

- **Pending agents:** Delete after 24 hours. These are agents whose approval flow was never completed — the user never visited the verification URL or the CIBA notification was ignored.
- **Pending hosts:** Delete after 7 days. A pending host represents an unknown client that attempted to register but was never approved.

Cleanup means deletion, not revocation — these principals never became active. Deletion removes the record entirely, freeing up any resources (pending approval flows, stored public keys).